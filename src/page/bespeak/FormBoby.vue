...
<script>
import CarNumer from '../carNumber/CarNumber' //导入车牌组件
...
export default {
  ...
  methods:{
    ...
    dateCancel(){
       ...
       this.beginDate = new Date(data[0],data[1]-1,data[2]) //重新传入年月日创建时间
    },
    ...
    timeConversion(StringTime){ //时间切割处理
        let data = StringTime.split("/")
        return data
    },

    updata(value){  //车牌前缀更改事件（父子组件传值）
        this.data.carprefix=value
    },

    CarShowEvent(){ //用户点击车牌前缀
        this.carShow=true
    },
    ...
    //门店选择
    ChoiceorgName(){
        if(this.datablank(this.data.lng,'获取定位失败,无法提供服务')){ //没有经度情况
            return
        }
